...
const encode = (obj: YarPacket) => {
  ...
  const packagerTypeBuf = Buffer.alloc(8);
  packagerTypeBuf.write(obj.packager, 0, PACKAGER_NAME_LEN, 'utf-8');
  ...
  return Buffer.concat([header, packagerTypeBuf, body], HEADER_LEN + PACKAGER_NAME_LEN + body.length);
};
...
const decode = (buf: Buffer): YarPacket => {
  ...
  const packagerType = buf.slice(HEADER_LEN, HEADER_LEN + PACKAGER_NAME_LEN).toString('utf-8').trim().replace(/\0/g, '');
  const packager = getPackager(packagerType);
